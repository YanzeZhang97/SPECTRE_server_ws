...
# Yanze
def extract_frames_v2(video_path, folderpath, detect_landmarks=True):
    videofolder = os.path.splitext(video_path)[0]
    os.makedirs(videofolder, exist_ok=True)
    vidcap = cv2.VideoCapture(video_path)
...
def main(args):
    args.crop_face = True
    args.input = '/home/yzhang94/Documents/ros2_ws/src/python_service/python_service/samples/LRS3/PbgB2TaYhio_00007.mp4'
    ...
    spectre.eval()
    image_paths, landmarks, videofolder, fps = extract_frames_v2(args.input, folderpath, detect_landmarks=args.crop_face)
